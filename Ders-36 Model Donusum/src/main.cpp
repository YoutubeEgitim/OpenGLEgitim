...
int main(int argc,char** argv)
{
    ...
    //Küp Oluşturuluyor
    m = new Model();
    mesh = MeshManager::getInstance()->createCone(8,8);
    m->m_Mesh = MeshManager::getInstance()->createGrid(0.2,20);
    ...
    glm::vec3 camPosition(0.0f,4.0f,4.0f);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        mesh->draw();
        mtxTransform = mtxProj*mtxCam*m->m_Transform->getWorldMatrix();
        
        program.setMat4("uMtxTransform",&mtxTransform);
        m->m_Mesh->draw();
        m->m_Transform->rotateX(1);
        m->m_Transform->setPosition(position);
