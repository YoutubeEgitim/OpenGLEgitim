...
Mesh* MeshManager::createCone(  float length,
                                    float radius,
                                    MESHDETAIL detail,
                                    const glm::vec4& topColor,
                                    const glm::vec4& surfaceColor)
{
    ...
    //vertex dizisinin boyutu başlangıçta bütün noktaları alacak kadar genişlitiliyor
    //bu sayede noktaları sıra ile eklemek zorunda kalmıyoruz.
    vertices.resize(circleVertexCount*2);
    ...
    for(int i = 0;i<circleVertexCount;i++)
    {
        ...
        //Önceclikle  dairenin nokta koordinatları hesaplanıyor
        float angle =(float) i*angleIncrese;
        vertex.pos.x = glm::cos(glm::radians(angle))*halfRadius;
        vertex.pos.z = glm::sin(glm::radians(angle))*halfRadius;

        vertex.pos.y = -(length/2.0f);
        std::cout<<vertex.pos.y<<std::endl;
        vertex.color = topColor;
        
        vertices[i] = vertex;

        //dairenin nokyaları ayrıca ara yüzeydede  kullanacağı için
        //vertex listesi içerisine aynı bir noktaya yerleştiriliyor
        //tabi renk değeri farklı veriliyor
        vertices[i+circleVertexCount] =vertex;
        vertices[i+circleVertexCount].color = surfaceColor ;

    }
    Vertex3Color topVertex;
    
    topVertex.pos = glm::vec3(0.0f,0.0f,0.0f);
    topVertex.pos.y = length/2.0f;
    std::cout<<vertices.size()<<std::endl;
    topVertex.color = surfaceColor;
    vertices.push_back(topVertex);

    int index = circleVertexCount;
    std::cout<<circleVertexCount<<std::endl;
    //dairelerin indeks değerleri ayarlanıyor 
    for(int i =0;i<circleVertexCount-1;i++)
    {
        //Ön Yüz 
...
Mesh* MeshManager::createGrid(float length,int gridCount)
{
    Vertex3ColorList vertices;
    IndexList indices;  
    float startX = -gridCount*length;
    float startZ = -gridCount*length;
    ...
    for(int i=-gridCount;i<=gridCount;i++)
    {
        ...
        {
            Vertex3Color vertex;
            vertex.pos.z = length*i;
            vertex.pos.x= length*j;
            vertex.pos.y = 0.0f;
            vertices.push_back(vertex);
            vertex.color = glm::vec4(0.3,0.3,0.3,1.0);
    ...
    for(int i=0;i<lineCount;i++)
    {
         ...
         {
            indices.push_back(i*lineCount+j);
            indices.push_back(i*lineCount+j+1);
          
            indices.push_back(j*lineCount+i);
            indices.push_back(j*lineCount+i+lineCount);
         }
    }
